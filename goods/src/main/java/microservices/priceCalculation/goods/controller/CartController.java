...
final class CartController {
    ...
    @GetMapping("/total")
    ResponseEntity<CTypeCalculatedEvent> getTotal(){

        /**id 需要从用户领域获得*/
        final String userId = "test1";

        CTypeCalculatedEvent calculatedEvent;
        try{

            calculatedEvent = cartService.getTotal(userId);

        }catch (Exception ex){

            return new ResponseEntity<CTypeCalculatedEvent>(HttpStatus.INTERNAL_SERVER_ERROR);
        }

        return ResponseEntity.ok(calculatedEvent);
    }
