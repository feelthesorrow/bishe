...
class CartServiceImpl implements CartService{
    ...
    private CartTotalRepository totalRepository;
    ...
    public CartServiceImpl(final CartRepository cartRepository1,
                           ...
                           final CartTotalRepository cartTotalRepository
    ){

        this.cartRepository = cartRepository1;
        this.totalRepository = cartTotalRepository;
    ...
    @Override
    @Transactional
    public CTypeCalculatedEvent getTotal(String userId) throws Exception{

        if(userId == null || "".equals(userId)){
            throw new Exception("user id cannot be null or empty.");
        }

        while (!totalRepository.existsById(userId)){

        }

        CTypeCalculatedEvent calculatedEvent = totalRepository.findByUserId(userId);
        totalRepository.delete(calculatedEvent);
    ...
    @Override
    @Transactional
    public CTypeCalculatedEvent saveTotal(CTypeCalculatedEvent cTypeCalculatedEvent) throws Exception{

        if(cTypeCalculatedEvent.getUserId() == null || "".equals(cTypeCalculatedEvent.getUserId())){
            throw new Exception("user id cannot be null or empty.");
        }

        return totalRepository.save(cTypeCalculatedEvent);
    }
}
